class Atm {
    ...
    abastecer(quantidade, valorab) {if ((valorab === 5)&&((this.comp1+quantidade)<=100)) { this.comp1 += quantidade; this.valor = this.comp1 * 5 + this.comp2 * 10 + this.comp3 * 20 + this.comp4 * 50 + this.comp5 * 100; } else {
				if(valorab === 10&&((this.comp2+quantidade)<=100)){
					this.comp2 += quantidade;
					this.valor = this.comp1 * 5 + this.comp2 * 10 + this.comp3 * 20 + this.comp4 * 50 + this.comp5 * 100;
				} else {
					if(valorab === 20&&((this.comp3+quantidade)<=100)){
						this.comp3 += quantidade;
						this.valor = this.comp1 * 5 + this.comp2 * 10 + this.comp3 * 20 + this.comp4 * 50 + this.comp5 * 100;
					} else { if (valorab === 50&&((this.comp4+quantidade)<=100)){
							this.comp4 += quantidade;
							this.valor = this.comp1 * 5 + this.comp2 * 10 + this.comp3 * 20 + this.comp4 * 50 + this.comp5 * 100;
                        } else { if (valorab === 100&&((this.comp5+quantidade)<=100)) { this.comp5 += quantidade;this.valor = this.comp1 * 5 + this.comp2 * 10 + this.comp3 * 20 + this.comp4 * 50 + this.comp5 * 100; } else {
                                return 0; }
                            }
					}
				}
...
let buttontotal = document.getElementById('constotal');
const tbody = document.querySelector('tbody');
buttontotal.addEventListener('click', function(){
	let valor = atm.valor;
	const tds = `<td class="valor">${valor}</td>`;
	const row = `<tr>${tds}</tr>`;
	tbody.innerHTML += row;
});

let buttonquantidade = document.getElementById('conscedulas');
buttonquantidade.addEventListener('click', function(){
	let compartimento1 = atm.comp1;
	let compartimento2 = atm.comp2;
	let compartimento3 = atm.comp3;
	let compartimento4 = atm.comp4;
	let compartimento5 = atm.comp5;
	const tds = `<td class="valor"> Notas de 5: ${compartimento1}  </td>
					<td class="valor"> Notas de 10: ${compartimento2} </td>
					<td class="valor"> Notas de 20: ${compartimento3} </td>
					<td class="valor"> Notas de 50: ${compartimento4} </td>
					<td class="valor"> Notas de 100: ${compartimento5} </td>` ;
	const row = `<tr>${tds}</tr>`;
	tbody.innerHTML += row;
});
